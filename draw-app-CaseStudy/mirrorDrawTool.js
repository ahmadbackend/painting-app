function mirrorDrawTool() {
	...
	this.draw = function() {
		...
		} else if (this.axis == "y"){                                          // will be modified to else x=y and add else for (45degree)
	...
	this.populateOptions = function() {
		...
		select("#directionButton").mouseClicked(function() {
			...
			if (self.axis == "x") {
				...
				button.html('Make 45degrees');
			}
            else{
                self.axis = "z";
                self.lineOfSymmetry = width / 9;  // need to establish a way to recalculate  from top left move to bottom right 
				button.html('Make horizontal');
            }
            // add another one for 45 degrees
